...
import static com.anadol.mindpalace.presenter.MyRandom.nextInt;
import static com.anadol.mindpalace.view.Dialogs.LearnStartBottomSheet.FORWARD;
import static com.anadol.mindpalace.view.Dialogs.LearnStartBottomSheet.INVERSE;
...
public class QuestionMaker {
    ...
    private Word[] getWordsForFalseAnswers(ArrayList<Word> wordArrayList, Word trueAnswer) {
        ...
        ArrayList<Integer> randomArrayList = MyRandom.getRandomInts(
                length,
                wordArrayList.size(),
    ...
    private Question createNewQuestion(Word word, Word[] wordsForFalseAnswers, int typeGroup, int route) {
        Question question;
        if (route == -1) route = MyRandom.nextInt(2);
